...
const yuCoinsTaskWeeklyModalRef = ref()
...
const queryList = ()=>{
  loading.value = true
  YuCoinsAPI.selectPageYuCoinsWeeklyInfo(yuCoinsQueryParams.value).then(request=>{
...
const add = () => {
  yuCoinsTaskWeeklyModalRef.value
...
const deletes = () => {
  ...
  ElMessageBox.confirm(
      ...
      .then(() => {
        let temList = []
        editList.map((n: YuCoinsTaskWeekly)=>temList.push(n.taskWeeklyId))
        YuCoinsAPI.deleteYuCoinsWeeklyInfo({taskWeeklyIds:temList}).then(request=>{
          if(request.code === 200){
...
const view = (row: YuCoinsTaskWeekly) =>{
...
const saveFormData=()=>{
  yuCoinsTaskWeeklyModalRef.value.save()
}
...
<template>
  ...
  <div style="display: flex; justify-content: flex-end; padding: 20px 0 0;">
    <el-pagination
        ...
        :total="formTotal"
        v-model:current-page="yuCoinsQueryParams.page"
    />
  </div>
  ...
  <el-dialog v-model="dialogFormVisible" :title="dialogName" width="800"  draggable  align-center destroy-on-close :close-on-click-modal="false">
    <YuCoinsTaskWeeklyModal :form-data-id="rowDataId" :is-edit="isEdit" ref="yuCoinsTaskWeeklyModalRef" @save="queryList"/>
    <template #footer>
